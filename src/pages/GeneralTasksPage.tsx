...
const GeneralTasksPage: React.FC = () => {
    ...
    getTasks()
      .then((res: IFrontendTask[]) => {
        if (Array.isArray(res)) {
          // Filter only General and Other tasks and exclude full tasks
          const generalTasks = res.filter((task: IFrontendTask) => {
            if (task.taskCategory !== 'General' && task.taskCategory !== 'Other') return false;
      ...
      .catch((_err) => {
        setError('Failed to load general tasks');
        setTasks([]);
      })
      ...
      <main className="max-w-6xl mx-auto my-10 p-4 pt-24">
        <div className="bg-white p-8 rounded-2xl shadow-lg">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold text-blue-700 mb-2">📋 General Tasks</h1>
              <p className="text-gray-600">General volunteer opportunities and community service tasks</p>
            </div>
            <button
              onClick={fetchTasks}
              disabled={loading}
              className="bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
              {loading ? 'Refreshing...' : '🔄 Refresh'}
            </button>
          </div>

          {loading && <p className="text-gray-500">Loading general tasks...</p>}
          ...
          {!loading && tasks.length > 0 && (
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-700 font-medium">
                📋 {tasks.length} General task{tasks.length !== 1 ? 's' : ''} available for volunteers
              </p>
            </div>
          )}
          ...
          <div className="space-y-6">
            {tasks.length === 0 && !loading ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg mb-2">No general tasks at the moment</p>
                <p className="text-gray-400 text-sm">Check back later for new volunteer opportunities</p>
              </div>
            ) : (
              tasks.map(task => (
                <div key={task._id} className="border border-gray-200 rounded-lg p-6 shadow-lg bg-gray-50">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-blue-800 mb-2">{task.title}</h3>
                    ...
                    if (currentUserAccepted) {
                      return (
                        <div className="mt-6 p-4 bg-green-100 border border-green-300 rounded-lg">
                          <p className="text-green-800 font-bold text-lg mb-3">✅ You have accepted this general task!</p>
                          {task.createdBy && (
                            <div>
                        ...
                        <div className="mt-6">
                          <div className="mb-4 p-3 bg-blue-100 border border-blue-300 rounded-lg">
                            <p className="text-blue-800 font-medium">
                              📋 <span className="font-bold">{availableSpots}</span> spot{availableSpots !== 1 ? 's' : ''} still available for this general task
                            </p>
                          </div>
                          <button 
                            className="w-full py-3 bg-blue-600 text-white rounded-lg font-bold text-lg hover:bg-blue-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed" 
                            onClick={() => handleAccept(task._id)} 
                            disabled={accepting === task._id}
                          >
                            {accepting === task._id ? 'Accepting...' : '📋 Accept General Task'}
                          </button>
                        </div>
                      );
                    }
